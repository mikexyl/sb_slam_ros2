"""Run the matched M2DGR Gate 1/2/3 MixVPR-512D 0.6 comparison.

This follows the ROS 1 M2DGRRS color-camera/handsfree-IMU sensor setup, while
using a 100-state backend and combined optical-flow/LightGlue frontend
tracking. It retains the JIST run's sequence construction, 0.1 boundary, and
distributed LightGlue verification. JIST frame refinement is disabled because
MixVPR does not expose the required five frame descriptors.
"""
...
def generate_launch_description():
    ...
    run_name = (
        "mixvpr0.6-512d-ds-noaug-lg-rematch-window100-realsense-mono-"
        "boundary0.1-consecutive-disabled-seqdiv-off-distlocal90-sim3-"
        "framerefine-off-"
        f"{timestamp}"
    )
    output_path = (
    ...
    recording_id = (
        "m2dgr_gate123_mixvpr06_rsmono_lg_window100_framerefine_off_"
        f"{timestamp}"
    )
    ...
    base_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "rerun_application_id": (
                "m2dgr_gate123_mixvpr06_rsmono_lg_window100_"
                "framerefine_off"
            ),
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "models.mixvpr",
                default_value=_default_model_path(
                    "mixvpr_resnet50_512d_fp16.engine"
                ),
            ),
