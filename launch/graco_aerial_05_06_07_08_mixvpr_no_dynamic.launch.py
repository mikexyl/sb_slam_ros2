"""Run the A5678 MixVPR-0.6 no-dynamic-sequence ablation.

All VIO, loop-closure, CBS, playback, and visualization settings come from the
proven augmented A5678 reference. MixVPR replaces JIST, uses the requested 0.6
database threshold, and retains every other admitted keyframe for its 4096-D
descriptor. Covisibility and self-similarity sequence gates are disabled in the
named VIO profile.
"""
...
def _run_identity():
    ...
    run_name = (
        "mixvpr-0.6-aug-no-dynamic-interval2-"
        f"distlocal90-sim3-{timestamp}"
    )
    output_path = workspace_root / "src" / "code-logs" / "a5678" / run_name
    recording_id = f"graco_a5678_mixvpr06_no_dynamic_i2_{timestamp}"
...
def generate_launch_description():
    ...
    reference_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "vio_dataset_name": "GrAcoStereoXfeatMixVprAugNoDynamicI2",
            "distributed_dataset_name": "GrAcoMixVprNoDynamic",
            "vpr_model_type": "mixvpr",
            "log_output": "true",
            "log_output_path": output_path,
            "rerun_application_id": "graco_a5678_mixvpr06_no_dynamic_i2",
