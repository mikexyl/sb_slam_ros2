"""Run M2DGR Gate 1/2/3 with the ROS 1 RealSense monocular setup.

The filtered ROS 2 bags retain the D435i color image and the handsfree IMU
used by ``code_slam_m2dgr_gate.launch``. One image-transport republisher per
robot decodes the JPEG color stream. Each active robot runs monocular VIO,
distributed loop closure, and Sim3 CBS; dense mapping is disabled.
"""
...
_SOURCE_IMAGE_TOPIC = "/camera/color/image_raw/compressed"
_SOURCE_IMU_TOPIC = "/handsfree/imu"


def _timestamped_recording_id():
    timestamp = datetime.now().astimezone().strftime("%Y%m%d_%H%M%S_%z")
    return f"m2dgr_gate_01_02_03_{timestamp}"
...
def _parse_active_robot_ids(value):
    ...
    if invalid:
        raise RuntimeError("active_robot_ids entries must be in [0, 3)")
    return robot_ids
...
def _image_republisher(robot_name):
    compressed_topic = f"/{robot_name}/camera/color/image_raw/compressed"
    raw_topic = f"/{robot_name}/camera/color/image_raw"
    return Node(
        package="image_transport",
        executable="republish",
        name=f"{robot_name}_color_decompressor",
...
def _bag_player(robot_name, bag_path, rate):
    target_image = f"/{robot_name}/camera/color/image_raw/compressed"
    target_imu = f"/{robot_name}/handsfree/imu"
    command = [
        "ros2",
        "bag",
        "play",
...
def _launch_setup(context, *args, **kwargs):
    ...
    if vio_mode != "mono":
        raise RuntimeError(
            "M2DGR Gate 1/2/3 must use the ROS 1 RealSense mono setup"
        )
    ...
    run_manifest = {
        "launch_file": "m2dgr_gate_01_02_03_multi_robot.launch.py",
            ...
            IncludeLaunchDescription(
                ...
                launch_arguments={
                    **shared_arguments,
                    ...
                    "right_image_topic": "",
                    "imu_topic": f"/{robot_name}/handsfree/imu",
                }.items(),
            )
        )
        actions.append(_image_republisher(robot_name))
        if play_bags:
            bag_players.append(
                _bag_player(
                    robot_name,
                ...
                TimerAction(
                    ...
                    actions=[
                        EmitEvent(
                            event=Shutdown(
                                reason=(
                                    "M2DGR Gate 1/2/3 smoke replay completed"
                                )
                            )
                        )
                    ],
...
def generate_launch_description():
    ...
    return LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                "vio_dataset_name",
                default_value="M2DGRRSXfeatJistDsNoAugNoLg",
            ),
            DeclareLaunchArgument(
                "distributed_dataset_name", default_value="M2DGR"
            ),
            ...
            DeclareLaunchArgument(
                "loop_closure.min_sim_vlad", default_value="0.8"
            ),
            DeclareLaunchArgument(
            ...
            DeclareLaunchArgument(
                "log_output_path",
                default_value="/tmp/sb_slam_ros2_logs/m2dgr_gate_123",
            ),
            DeclareLaunchArgument(
                "rerun_application_id",
                default_value="m2dgr_gate_01_02_03_multi_robot",
            ),
            DeclareLaunchArgument(
                "rerun_recording_id",
