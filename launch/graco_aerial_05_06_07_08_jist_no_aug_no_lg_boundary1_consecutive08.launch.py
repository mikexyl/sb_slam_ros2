"""Run the A5678 JIST no-augmentation/no-LightGlue-rematch ablation."""
...
def _run_identity():
    ...
    workspace_root = Path(os.environ.get("SB_SLAM_ROS2_WS", os.getcwd()))
    run_name = (
        "jist-noaug-nolgrematch-boundary1-consecutive0.8-"
        "jist0.7-diversity-off-distlocal90-sim3-"
        f"{timestamp}"
    )
    output_path = workspace_root / "src" / "code-logs" / "a5678" / run_name
    recording_id = (
        f"graco_a5678_jist_noaug_nolg_b1_c08_{timestamp}"
    )
...
def generate_launch_description():
    ...
    reference_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "bag_rate": "1.0",
            "vio_dataset_name": (
                "GrAcoStereoXfeatJistNoAugNoLgBoundary1Consecutive08"
            ),
            ...
            "rerun_application_id": (
                "graco_a5678_jist_noaug_nolg_b1_c08"
            ),
