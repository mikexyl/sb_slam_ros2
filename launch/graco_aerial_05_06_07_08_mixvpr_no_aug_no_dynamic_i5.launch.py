"""Run the A5678 MixVPR-0.6 no-augmentation, stride-five ablation.

All VIO, loop-closure, CBS, playback, and visualization settings come from the
completed augmented interval-five run. The named VIO profile changes only
landmark projection to disabled. MixVPR uses the 0.6 database threshold and
processes frame IDs 0, 5, 10, ... for its 4096-D descriptor; covisibility and
self-similarity sequence gates remain disabled.
"""
...
def _run_identity():
    ...
    run_name = (
        "mixvpr-0.6-noaug-no-dynamic-interval5-"
        f"distlocal90-sim3-{timestamp}"
    )
    output_path = workspace_root / "src" / "code-logs" / "a5678" / run_name
    recording_id = f"graco_a5678_mixvpr06_noaug_no_dynamic_i5_{timestamp}"
...
def generate_launch_description():
    ...
    reference_launch = IncludeLaunchDescription(
        ...
        launch_arguments={
            ...
            "bag_rate": "1.0",
            "vio_dataset_name": "GrAcoStereoXfeatMixVprNoAugNoDynamicI5",
            "distributed_dataset_name": "GrAcoMixVprNoDynamic",
            "vpr_model_type": "mixvpr",
            "log_output": "true",
            "log_output_path": output_path,
            "rerun_application_id": "graco_a5678_mixvpr06_noaug_no_dynamic_i5",
